...
import { ImportEntry, ModuleIR } from './types/ir';
...
export function parsePyFile(file: string, workspaceRoot: string): ModuleIR {
  ...
  console.log('from *parse_py.ts* [server][py] imports', {
...
function resolvePythonImport(
  ...
  if (source) {
    const parts = source.split('.');
    const base = path.resolve(root, ...parts);
  ...
  if (source && specifiers.length) {
    const baseParts = source.split('.');
    for (const name of specifiers) {
      const sub = path.resolve(root, ...baseParts, name);
      candidates.push(sub + '.py');
      candidates.push(path.join(sub, '__init__.py'));
    }
  }
  ...
  if (!source && specifiers.length) {
    ...
    for (const name of specifiers) {
      const sub = path.resolve(dir, name);
      candidates.push(sub + '.py');
      candidates.push(path.join(sub, '__init__.py'));
    }
  }
